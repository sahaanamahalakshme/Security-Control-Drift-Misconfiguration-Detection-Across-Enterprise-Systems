"""
backend/attack_graph/graph_builder.py

Attack Graph Builder.

Input : topology.json — nodes have {id, type, provider, environment,
        criticality, exposed_to_internet, business_tag}; edges use
        {"from", "to", "relationship", mitre_technique,
        exploit_probability, compliance_impact, business_impact}.
        baseline.json optionally overlays control_id onto matching
        resource_id nodes (safe to omit — code degrades gracefully).

Output: a directed graph (networkx.DiGraph) with edge weights carrying
        exploit_probability, plus reachability/blast-radius helpers.
...
from shared.models import GraphNode, GraphEdge
